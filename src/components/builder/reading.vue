    ...
    <div v-show="!previewing">
        ...
        <Transition>
            <div  v-if="configurationData">
                <label class="block w-fit py-1 px-5 rounded-t bg-slate-200 mx-auto">Reading</label>
                <div class="bg-slate-200 p-1 rounded bg-secondary">
                    <textarea v-model="odablocks.reading" class="block border-2 text-base bg-white p-2  border-secondary rounded w-full min-h-[120px]" placeholder=""></textarea>
                </div>
...
const buildODA = () => {
    ...
    let qoRead = JSON.parse(JSON.stringify(readingObject))
    qoRead.content[0].text = odablocks.value.reading
    defdoc.activity.scenes[0].content[0].content.push(qoRead)
